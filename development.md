...
- دکمه‌های شناور تماس و اینستاگرام باید در دو سوی صفحه، بیرون از محدوده محتوای اصلی و با رعایت `safe-area` موبایل باقی بمانند.
- انیمیشن باید `prefers-reduced-motion` را رعایت کند.
- از Cardهای تودرتو و شلوغی بصری خودداری کنید.
- برای تصاویر اصلی از WebP بهینه استفاده کنید و نسبت تصویر و `sizes` را متناسب با قاب واکنش‌گرا نگه دارید.
- تصاویر تولیدی یا تحریریه‌ای باید در متن جایگزین و رابط با عنوان «تصویر مفهومی» از رسانه واقعی مرکز تفکیک شوند.

### هویت بصری و لوگو

- فایل مادر نشان در `public/brand/logo-mark.svg` قرار دارد و باید تنها منبع ویرایش لوگو باشد.
- مفهوم نشان، خورشید در آستانه‌ای رو به یک مسیر تازه است؛ رنگ‌های اصلی آن سبز جنگلی `#123F38`، عاجی `#FFF8E8` و زعفرانی `#F5B942` هستند.
- مؤلفه `src/components/Brand.tsx` نشان برداری را کنار نام و توضیح مرکز نمایش می‌دهد؛ متن برند را داخل SVG قرار ندهید تا دسترس‌پذیر و قابل ترجمه بماند.
- پس از هر تغییر در فایل مادر، `npm run brand:icons` را اجرا کنید. این فرمان `favicon.ico`، آیکن Apple و پیش‌نمایش ۵۱۲ پیکسلی را از همان SVG بازتولید می‌کند.
- جزئیات تازه‌ای به نسخه favicon اضافه نکنید؛ نشان باید در اندازه ۱۶ پیکسل نیز قابل تشخیص بماند.

مرجع طراحی در `docs/design/homepage-concept.png` و `docs/design/admin-concept.png` نگهداری می‌شود.

تصاویر اختصاصی نسخه فعلی در `public/images` قرار دارند:

- `hero-threshold-v2.webp`: تصویر مفهومی قهرمان صفحه
...
- `center-management.webp`: تصویر واقعی سید سجاد هاشمی، مدیر مرکز؛ فایل اصلی در ۱۴۰۵/۰۶/۰۸ توسط مالک پروژه برای انتشار ارائه شده است. هنگام جایگزینی، مجوز انتشار و متن جایگزین دقیق حفظ شود.
...
### واردکردن محتوای شبکه‌های اجتماعی

محتوای منتخب صفحه رسمی مرکز به‌صورت خودکار از اینستاگرام Hotlink نمی‌شود. فایل مجاز ابتدا دریافت و برای وب به MP4 یا JPEG/PNG/WebP بهینه می‌شود و سپس از `/admin/media` بارگذاری می‌گردد. این روش وابستگی سایت به دسترسی اینستاگرام، انقضای URLهای CDN و افت سرعت را حذف می‌کند.

ویدیوهای MP4 و WebM هنگام بارگذاری با FFmpeg به MP4 شامل H.264 Main، صدای AAC، فضای رنگ `yuv420p` و `faststart` تبدیل می‌شوند. خطای Decode باعث ردشدن فایل و پاک‌سازی خروجی ناقص می‌شود. در توسعه محلی باید فرمان `ffmpeg` در `PATH` در دسترس باشد؛ ایمیج Docker آن را نصب می‌کند.

- منبع و تاریخ دریافت در توضیح رسانه ثبت شود.
- تصاویر و ویدیوهای محیط مرکز در دسته «فعالیت مرکز» قرار گیرند.
- حضور چهره یا روایت شخصی به معنی رضایت برای انتشار در وب‌سایت نیست؛ برای دسته «روایت بهبودی» ثبت رضایت قابل اثبات الزامی است.
- نام کاربری، گذرواژه، Cookie یا اطلاعات پراکسی در پروژه و Audit Log ذخیره نشود.
...
### ویدیو پخش نمی‌شود یا تصویر به‌هم‌ریخته است

- وجود FFmpeg را با `ffmpeg -version` بررسی کنید.
- پاسخ `/media/<storageKey>` باید `Content-Type: video/mp4`، `Accept-Ranges: bytes` و برای درخواست Range وضعیت 206 داشته باشد.
- فایل‌های قدیمی را با `ffmpeg -v error -xerror -i FILE.mp4 -f null -` اعتبارسنجی کنید؛ هر خروجی خطا نشانه نیاز به جایگزینی از نسخه اصلی سالم است.
- از بازنویسی صرف Container بدون پشتیبان Volume رسانه خودداری کنید.
