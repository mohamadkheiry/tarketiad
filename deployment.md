...
این فرمان خدمات، پرسش‌ها، متن‌های اصلی و اطلاعات هویتی مرجع مرکز را بازنویسی می‌کند، اما درخواست‌های مشاوره را حذف نمی‌کند. سپس صفحه اصلی و پنل محتوا را بررسی کنید.

## نگهداری رسانه‌های برگرفته از صفحه رسمی مرکز

فایل‌های اینستاگرام پس از تأیید مالک محتوا باید از پنل رسانه بارگذاری شوند و در Volume دائمی نگهداری می‌شوند. سایت نباید به URL موقت CDN اینستاگرام وابسته باشد. هنگام جایگزینی یا حذف یک پست در شبکه اجتماعی، نسخه وب‌سایت نیز در `/admin/media` بازبینی شود. هیچ Credential یا تنظیم پراکسی روی سرور پروژه ذخیره نشود.

ایمیج برنامه FFmpeg را برای تبدیل خودکار ویدیو به H.264/AAC نصب می‌کند. بعد از هر به‌روزرسانی مسیر رسانه، سلامت ابزار و پاسخ Range را بررسی کنید:

```bash
docker compose exec -T app ffmpeg -version
curl -I -H 'Range: bytes=0-1023' http://127.0.0.1:8012/media/STORAGE_KEY.mp4
```

پاسخ Range باید `206` و دارای `Content-Range` باشد. مقدار `STORAGE_KEY` را از پنل رسانه یا دیتابیس بردارید و هیچ مسیر فایل یا Credential را در لاگ عمومی منتشر نکنید.
